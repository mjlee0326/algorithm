...
struct cir_list{
	...
	public:
		cir_list() : n(0){
			head = new node {NULL, NULL, NULL}; //모두 NULL로 구성된 node
			head -> next = head;;
			head -> prev = head;
		}

		size_t size() const{ //body 시작 직전에 const: 해당 함수가 속한 객체의 멤버 변경 불가
			return n;
		}

		void insert(const T& value){
			//T 자료형으로 동적할당을 하고 value값으로 초기화
